...
def _shard_map(fn, out_specs):
  mesh = jax.make_mesh(
      (jax.device_count(),), ("x",), axis_types=(AxisType.Explicit,)
  )
...
class StateDischargeTest(jtu.JaxTestCase):
  ...
  def test_partial_fori_discharge(self):
    ...
    ref = lambda x: AbstractRef(core.typeof(x))
...
class StateControlFlowTest(jtu.JaxTestCase):
  ...
  @parameterized.named_parameters(
      ("jit", "jit"), ("remat", "remat"), ("shard_map", "shard_map")
  )
  ...
  def test_grad_with_ref_view_write(self):
    @jax.jit
    def f(x):
      ref = jax.new_ref(jnp.zeros(5))
      ref.at[1:4][...] = x
      return ref[...].sum()

    x = jnp.array([1.0, 2.0, 3.0])
    g = jax.grad(f)(x)
    self.assertAllClose(g, jnp.ones(3))

  @absltest.skip(
      "TODO: addupdate transpose produces view-shaped cotangent vs full-ref"
      " accumulator"
  )
  def test_grad_with_ref_view_addupdate(self):
    @jax.jit
    def f(x):
